 ...
 <ul>
  <li>
   <dl>
    ...
    <dd><a href="/store/CateServlet?actionselectall" class="active">商品分类</a></dd>
      ...
      <!--左右分栏：左侧栏目-->
       ...
       <div algin="center">
           <table class="table fl">
            ...
            <% List list = (List) session.getAttribute("catelist");
				Iterator it = list.iterator();
				int i=1;
				while (it.hasNext()) { 
				Category cate=(Category)it.next();
				%>
				<form action="/store/CateServlet">
				<input type="hidden" name="action" value="update">
				<input type="hidden" name="cid" value=<%=cate.getCid() %>>
            <tr>
            <td><%=i++%></td>
             <td class="center"><%=cate.getName() %></td>
             <td><input type="text" name="cdesc" value=<%=cate.getCdesc() %>></td>
            <td> <input type="submit" value="修改"></form>
             <a href="/store/CateServlet?action=delete&cid=<%=cate.getCid() %>" class="inner_btn">删除</a></td>
            </tr>
           <%} %>
           </table>
       </div>

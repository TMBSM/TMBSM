 ...
 <ul>
  ...
  <li>
   <dl>
    ...
    <dd><a href="/store/MLServlet?action=selectall"  class="active">会员等级</a></dd>
   </dl>
  </li>
      ...
      <!--左右分栏：左侧栏目-->
       ...
       <div algin="center">
           <table class="table fl">
            ...
            <% List list = (List) session.getAttribute("MLlist");
				Iterator it = list.iterator();
				int i=1;
				while (it.hasNext()) { 
				Mlevel ml=(Mlevel)it.next();
				%>
				<form action="/store/MLServlet">
				<input type="hidden" name="action" value="update">
				<input type="hidden" name="lid" value=<%=ml.getLid() %>>
            <tr>
            <td><%=i++%></td>
             <td class="center"><%=ml.getLevelName() %></td>
             <td><input type="text" name="favourable" value=<%=ml.getFavourable() %>></td>
            <td> <input type="submit" value="修改"></form>
             <a href="/store/MLServlet?action=delete&lid=<%=ml.getLid() %>" class="inner_btn">删除</a></td>
            </tr>
           <%} %>
           </table>
       </div>
